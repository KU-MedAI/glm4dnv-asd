...
export WANDB_PROJECT="${WANDB_PROJECT:-ntv2_ft_ncre}"

: "${RUN_ROOT:=/path/to/experiments/ntv2/ncre}"
export RUN_ROOT

# Paths
: "${BASE_DIR:=/fine-tuned/Nucleotide-Transformer-V2/regression}"
: "${DATA_PATH:=/path/to/data/NCRE_activity_filtered.parquet}"
...
PY="${BASE_DIR}/ft_ntv2_regression.py"
...
# =========================================================
AGENT_SCRIPT="${BASE_DIR}/wandb_agent_runner_ncre.sh"
...
export RUN_ROOT
export AGENT_SCRIPT FT_OUTPUT_ROOT FT_LOG_ROOT DATA_PATH SEQ_COL LABEL_COL CHECKPOINT PY
...
# =========================================================
# 4) Pick BEST by test_PCC (ONLY ONE best_run.txt in ft/output)
# =========================================================
...
for p in paths:
    try:
        ...
        if not pd.isna(pcc) and pcc > best_score:
            best_score = pcc
            best_p = p
...
if best_p is None:
    print("No valid test_metrics.csv found, trying SCC as backup...")
    for p in paths:
        try:
            df = pd.read_csv(p)
            scc = float(df.loc[0, "test_SCC"])
            if not pd.isna(scc) and scc > best_score:
                best_score = scc
                best_p = p
        except Exception:
            pass
...
if best_p is None:
    raise SystemExit("No valid test_metrics.csv found with PCC or SCC")
