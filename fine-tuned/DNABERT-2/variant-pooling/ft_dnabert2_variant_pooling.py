...
class DNABERT2EmbedderMutMean:
    ...
    def __init__(
        ...
        print(f"Loading Standard BertConfig for {base_ckpt}...")
        ...
        w_before = first_param.clone().detach()
        print(f"[Check] Base model loaded. Sample weight mean: {w_before.mean().item():.8f}")

        if ft_pth_path and str(ft_pth_path).strip():
            print(f"Loading custom weights from: {ft_pth_path}")
            ...
            w_after = next(base_model.parameters())
            diff = torch.sum(torch.abs(w_after - w_before)).item()
            
            if diff > 1e-9:
                print(f"[Verified] Weights UPDATED successfully! (Diff: {diff:.8f})")
                print(f"   - Before mean: {w_before.mean().item():.8f}")
                print(f"   - After mean : {w_after.mean().item():.8f}")
            else:
                print(f"[WARNING] Weights appear UNCHANGED after loading .pth!")
                print(f"   - Check if the keys in .pth match the model definition.")



        
        if lora_adapter_dir and str(lora_adapter_dir).strip():
            print(f"[INFO] Loading LoRA adapter from: {lora_adapter_dir}")
            ...
            lora_layers = [n for n, _ in model.named_modules() if "lora" in n.lower()]
            if len(lora_layers) > 0:
                print(f"[Verified] LoRA adapter attached successfully.")
                print(f"   - Found {len(lora_layers)} LoRA layers in the model.")
            else:
                print(f"[WARNING] No 'lora' layers found in the model structure!")

            if merge_lora:
                print("Merging LoRA weights into base model...")
                model = model.merge_and_unload()
                
                w_merged = next(model.parameters())
                diff_merge = torch.sum(torch.abs(w_merged - w_before)).item()
                if diff_merge > 1e-9:
                     print(f"[Verified] LoRA merged successfully! Base weights updated. (Diff: {diff_merge:.8f})")
                else:
                     print(f"[WARNING] Weights unchanged after LoRA merge. (Maybe LoRA weights were zero?)")
        ...
        if isinstance(self.model, PeftModel):
            ...
            self.backbone = getattr(self.model, "bert", getattr(self.model, "student", None))
            
        self.dim = int(getattr(self.backbone.config, "d_model", 
                              getattr(self.backbone.config, "hidden_size", 768)))
        print(f"Model initialization complete. Hidden dim: {self.dim}")
    ...
    def _encode_to_per_base(self, seqs: List[str]) -> List[np.ndarray]:
        ...
        for b in range(len(seqs)):
            ...
            for i, (start, end) in enumerate(offsets[b]):
                if start == end: continue
                base_res_emb[start:end] = hidden[b, i, :]
    ...
    def get_mut_embeddings_from_df(
        ...
        out_rows = []
        n = len(seq_df)
        results = np.zeros((n,self.dim),dtype = np.float32)

        with torch.inference_mode():
            for start in tqdm(range(0, n, self.batch_size), desc="DNABERT-2 mutation pooling (max, per-base)"):
                ...
                for i, (seq_emb, mut_idx_list) in enumerate(zip(per_base_embs, mut_lists)):
                    ...
                    results[global_idx] = pooled.astype(np.float32)
        ...
        return pd.DataFrame({out_col: list(results)})
...
def main():
    ...
    embedder = DNABERT2EmbedderMutMean(
