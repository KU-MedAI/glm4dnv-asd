...
export WANDB_PROJECT="${WANDB_PROJECT:-phylogpn_ft_bend}"

: "${RUN_ROOT:=/path/to/experiments/phylogpn/bend}"
export RUN_ROOT

# Paths
: "${BASE_DIR:=/fine-tuned/PhyloGPN/classification}"
...
PY="${BASE_DIR}/ft_phylogpn_classification.py"
...
cfg = {
    ...
    "command": ["bash", AGENT_SCRIPT, "${args}"]
}
